...
class ModelArguments:
    ...
    unfreeze_layers: Optional[List[str]] = field(
        default=None, metadata={"help": "Which layer(s) to unfreeze"})
...
def load_args():
    ...
    s_path = Path(sys.argv[3]) if len(sys.argv) > 3 else None

    v_name = v_path.name.split('.')[0]
    d_name = d_path.name.split('.')[0]
    if s_path is not None:
        s_lang, s_name = s_path.name.split('.')[0].split('_')
        assert s_lang == d_name.split('_')[-1]
        d_name = f'{d_name}-{s_name}'
    ...
    if os.path.exists(args['output_dir']) and args['overwrite_output_dir']:
        ...
        if ckpt == 0:
            print(f'WARNING: output {args["output_dir"]} dir exists, but does not contain a checkpoint')
        else:
...
def main():
    ...
    if model_args.unfreeze_layers is not None:
        for name, p in model.named_parameters():
            ...
            if name in model_args.unfreeze_layers:
                p.requires_grad = True
                continue
            for lay_name in model_args.unfreeze_layers:
    ...
    if data_args.line_by_line:
        ...
        def group_texts(examples):
            print(f'grouping {len(examples):,} texts')
