...
for treebank_name in ['GUM', 'ParTUT']:
    src_dir = Path(
        'data/sources') / 'ud-treebanks-v2.7' / f'UD_English-{treebank_name}'

    tgt_dir = Path('data/examples')
    os.makedirs(tgt_dir.parent, exist_ok=True)

    for fold in ['train', 'dev', 'test']:
        src_path = src_dir / f'en_{treebank_name.lower()}-ud-{fold}.conllu'
        tgt_path = tgt_dir / f'en_{treebank_name.lower()}_{fold}.json'
